...
class manageProfessionals extends Component {
    ...
    async flagCustomer(id){

        await this.showLoader();

        await fetch(env.api + "admin/flag/professional/"+ id +"/" + this.state.admin.adminId, {
            method: "PUT",
            headers: {
                "Authorization": 'Bearer ' + this.state.token
            }
    ...
    async fetchProfessionals() {
        await fetch(env.api + "admin/list/professionals", {
            method: "GET",
            headers: {
                "Authorization": 'Bearer ' + this.state.token
            }
            ...
            .then(async data => {
                if (data.error) {
                    ...
                    await professionals.sort(function (a, b) {
    ...
    render() {
        ...
        let data = null;
        if (this.state.professionals !== null) {
            data = this.state.professionals.map(user => {
                return (
                    <Card key={user._id} containerStyle={styles.card}>
                        ...
                        <View style={styles.briefBottom}>
                            <TouchableOpacity style={styles.rescheduleBtn}
                                onPress={() => {
                                    this.props.navigation.navigate('manageProfessionalDetails', { user: user });
                                }}
                            >
        ...
        return (
            <View style={styles.container}>
                <View style={styles.header}>
                    ...
                    />
                    <Text style={styles.headerText}>Manage Professionals</Text>
